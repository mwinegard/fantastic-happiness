...
(() => {
  ...
  // ---------------- Specials: interactive modals ----------------
  function cardLabel(c){
    const color = String(c.color || "").toUpperCase();
    const type = String(c.type || "").replace(/_/g," ").toUpperCase();
    const val = (typeof c.value === "number") ? ` ${c.value}` : "";
    return `${color} ${type}${val}`.trim();
  }

  function makeCardTile(cardSnap, { selected=false, onClick } = {}){
    const b = document.createElement("button");
    b.type = "button";
    b.className = "fh-btn fh-btn-ghost";
    ...
    b.style.border = selected ? "2px solid rgba(56,189,248,.85)" : "1px solid rgba(255,255,255,.14)";
    b.style.background = "rgba(0,0,0,.18)";

    const img = document.createElement("img");
    img.alt = cardLabel(cardSnap);
    img.src = `assets/cards/${imgFromSnap(cardSnap)}`;
    ...
    cap.textContent = cardLabel(cardSnap);
    cap.style.fontSize = "11px";
    cap.style.fontWeight = "900";
    cap.style.opacity = "0.85";
    cap.style.textAlign = "center";
    cap.style.lineHeight = "1.1";
  ...
  socket.on("shoppingChooseTarget", ({ targets }) => {
    ...
    p.style.opacity = "0.9";
    p.textContent = "Pick a player to shop with:";
    body.appendChild(p);
    ...
    const modal = openModal("🛒 Shopping", body, [
  ...
  socket.on("shoppingPickGive", ({ hand }) => {
    ...
    const intro = document.createElement("div");
    intro.style.opacity = "0.9";
    intro.textContent = "Choose TWO cards to give:";
    body.appendChild(intro);
    ...
    function redraw(){
      for (const [i, el] of tiles.entries()){
        ...
        el.style.border = sel ? "2px solid rgba(56,189,248,.85)" : "1px solid rgba(255,255,255,.14)";
        el.style.transform = sel ? "translateY(-2px)" : "";
      }
    }

    cards.forEach(c => {
      const idx = Number(c?.i);
      const tile = makeCardTile(c, {
        selected: false,
        onClick: () => {
          if (chosen.has(idx)) chosen.delete(idx);
          else {
            if (chosen.size >= 2) return toast("Pick exactly 2 cards.");
            chosen.add(idx);
          }
          redraw();
        }
    ...
    const modal = openModal("🛒 Shopping — Give 2", body, [
      ...
      { label: "Confirm", primary: true, onClick: ({ close }) => {
        if (chosen.size !== 2) return toast("Pick exactly 2 cards.");
        const [a,b] = Array.from(chosen.values());
        socket.emit("shoppingGiveChosen", { idx1: a, idx2: b });
        close();
      }}
    ]);
  });
  ...
  socket.on("shoppingPickTake", ({ hand }) => {
    ...
    const intro = document.createElement("div");
    intro.style.opacity = "0.9";
    intro.textContent = "Choose ONE card to take:";
    body.appendChild(intro);
    ...
    cards.forEach(c => {
      ...
      const tile = makeCardTile(c, {
        selected: false,
        onClick: () => { chosen = (chosen === idx) ? null : idx; redraw(); }
      });
      tiles.set(idx, tile);
      grid.appendChild(tile);
    });
    ...
    const modal = openModal("🛒 Shopping — Take 1", body, [
      { label: "Cancel", primary: false, onClick: ({ close }) => close() },
      { label: "Confirm", primary: true, onClick: ({ close }) => {
        if (chosen == null) return toast("Pick 1 card.");
  ...
  socket.on("promiseChooseTarget", ({ targets }) => {
    ...
    const p = document.createElement("div");
    p.style.opacity = "0.9";
    p.textContent = "Pick a player to Pinky Promise with:";
    body.appendChild(p);
  ...
  socket.on("rainbowPick", ({ hand }) => {
    ...
    intro.innerHTML = `Pick <b>one RED</b>, <b>one YELLOW</b>, <b>one GREEN</b>, and <b>one BLUE</b> card to discard (then choose the next active color).`;
    body.appendChild(intro);

    const grid = makeGrid();
    body.appendChild(grid);

    const chosenByColor = new Map(); // color -> index
    const idxToSnap = new Map();
    const tiles = new Map();

    function redraw(){
      for (const [idx, el] of tiles.entries()){
        const snap = idxToSnap.get(idx);
        const col = String(snap?.color || "").toLowerCase();
        const sel = chosenByColor.get(col) === idx;
        el.style.border = sel ? "2px solid rgba(56,189,248,.85)" : "1px solid rgba(255,255,255,.14)";
        el.style.transform = sel ? "translateY(-2px)" : "";
      }
    }
    ...
    cards.forEach(c => {
      ...
      const tile = makeCardTile(c, {
        onClick: () => {
          const col = String(c?.color || "").toLowerCase();
          if (!["red","yellow","green","blue"].includes(col)) return toast("Rainbow picks must be Red/Yellow/Green/Blue.");
          const existing = chosenByColor.get(col);
          if (existing === idx) chosenByColor.delete(col);
          else chosenByColor.set(col, idx);
          redraw();
        }
      });
      tiles.set(idx, tile);
    ...
    const modal = openModal("🌈 Rainbow — Pick 4 Colors", body, [
      { label: "Cancel", primary: false, onClick: ({ close }) => close() },
      { label: "Confirm", primary: true, onClick: ({ close }) => {
        const need = ["red","yellow","green","blue"];
        const missing = need.filter(c => !chosenByColor.has(c));
        if (missing.length) return toast(`Missing: ${missing.map(x=>x.toUpperCase()).join(", ")}`);
        const indices = need.map(c => chosenByColor.get(c));
  ...
  socket.on("lookTop", ({ cards }) => {
    ...
    const topFirst = list.slice().reverse(); // server indexes are bottom->top; show top->bottom
    ...
    const intro = document.createElement("div");
    intro.style.opacity = "0.92";
    intro.innerHTML = `Tap the cards in the order you want them drawn (<b>TOP → BOTTOM</b>).`;
    body.appendChild(intro);
    ...
    function redraw(){
      pickRow.innerHTML = "";
      chosenTop.forEach(ix => {
        const snap = list.find(x => Number(x.i) === ix);
        const pill = document.createElement("div");
        pill.className = "fh-badge";
        pill.style.padding = "8px 10px";
        pill.style.borderRadius = "999px";
        pill.textContent = snap ? cardLabel(snap) : `#${ix}`;
        pickRow.appendChild(pill);
      });

      // fade chosen cards
      grid.querySelectorAll("button[data-peek]").forEach(btn => {
        const ix = Number(btn.getAttribute("data-peek"));
        btn.style.opacity = chosenSet.has(ix) ? "0.55" : "1";
      });
    }
    ...
    topFirst.forEach(c => {
      ...
      const tile = makeCardTile(c, {
        onClick: () => {
          if (chosenSet.has(ix)) return;
          chosenSet.add(ix);
          chosenTop.push(ix);
          redraw();
        }
      });
      tile.setAttribute("data-peek", String(ix));
      grid.appendChild(tile);
    });

    const modal = openModal("👀 Blue Look — Reorder", body, [
      { label: "Reset", primary: false, onClick: () => { chosenTop.length = 0; chosenSet.clear(); redraw(); } },
      { label: "Keep as-is", primary: false, onClick: ({ close }) => {
        socket.emit("lookTopOrder", { order: list.map(x => x.i) }); // bottom->top original
        close();
      }},
      { label: "Confirm", primary: true, onClick: ({ close }) => {
        if (chosenTop.length !== list.length) return toast(`Pick ${list.length} cards (top → bottom).`);
        // Server expects bottom->top order.
        const order = chosenTop.slice().reverse();
