...
// ✅ Spectator/Admin never takes a seat (server-enforced via join {spectator:true})
// ✅ Card image references fixed to match your asset filenames (*_draw.png not *_draw2.png)
// ✅ Adds missing yellow_pinkypromise card into the deck
// ✅ Adds sound events for your sound pack (draw/skip/reverse/wild/special/number/win/lose/start/joined/uno)
// ✅ Prevents undefined cards from entering hands if deck runs out
...
function emitSoundPerPlayer(L, winnerSid){
  // round end: winner gets win, everyone else gets lose
  for (const p of L.players) {
    if (!p?.id) continue;
    if (p.spectator) continue; // spectators can still hear if you want; set to false to include them
...
function makeDeck(){
  ...
  for (const color of COLORS){
    ...
    deck.push({ color:"yellow", type:"yellow_pinkypromise", img:`yellow_pinkypromise.png`}); // ✅ was missing
...
function deal(L){
  ...
  if (!first) {
    announce(L, "⚠️ Could not start: deck empty.");
    G.started = false;
    return;
  }
...
io.on("connection", (socket)=>{
  ...
  socket.on("join", ({ name, lobby, spectator })=>{
    ...
    // Ensure spectators NEVER have a seat/hand
    const G = L.game;
    G.hands = G.hands || {};

    if (p.spectator) {
      if (G.hands[p.sid] && G.hands[p.sid].length) {
        reshuffleCardsIntoDeck(G, G.hands[p.sid]);
      }
      delete G.hands[p.sid];
    } else {
      // If joining mid-game as seated, ensure a hand exists
      if (G.started && !Array.isArray(G.hands[p.sid])) {
        G.hands[p.sid] = drawCards(G, START_HAND);
    ...
    // Start game only if at least 2 SEATED players exist
    if (!L.game.started && seatedPlayers(L).length>=2){
      ...
      // If current is now invalid (e.g. current left or became spectator), repair it
      if (L.game.started) {
        const order = activeOrder(L);
        if (!order.length) {
          L.game.started = false;
        } else if (!order.includes(L.game.current)) {
          beginTurn(L, order[0]);
        }
      }
      emitState(L);
  ...
  socket.on("drawCard", ()=>{
    ...
    if (G.pendingPenalty && G.pendingPenalty.targetSid===me.sid){
      ...
      const drawn = drawCards(G, G.pendingPenalty.amount);
      const hand = G.hands[me.sid] || (G.hands[me.sid] = []);
      hand.push(...drawn);

      announce(L, `😵 ${me.name} drew ${drawn.length} (stack ended).`);
    ...
    if (!card) { io.to(me.id).emit("warn","No cards left to draw."); return; }
  ...
  socket.on("admin:forceRoundEnd", () => {
    ...
    const hands = Object.entries(G.hands || {}).map(([sid,h])=>({sid, n:(h||[]).length}))
      .filter(x => activeOrder(L).includes(x.sid));
  ...
  socket.on("playCard", ({ index })=>{
    ...
    if (card.type==="draw2"){
      // keep top state consistent
      G.color = card.color; G.value = "draw2";
      emitSoundToRoom(L, "wild"); // your list doesn't include draw2; wild is a decent "power" sound
    ...
    if (card.type==="red_noc"){
      ...
      if (!G.nocTarget){
        ...
        const drew = drawCards(G,3);
        (G.hands[chosen.sid] = G.hands[chosen.sid]||[]).push(...drew);

        announce(L, `📄 NOC Notice issued! ${chosen.name} has been flagged and must draw ${drew.length}.`);
      } else {
        ...
        if (!target){
          ...
          const drew = drawCards(G,3);
          (G.hands[target.sid]=G.hands[target.sid]||[]).push(...drew);
          announce(L, `📄 NOC Notice reminder: ${target.name} is still flagged and draws ${drew.length} more.`);
        }
      }
    ...
    if (card.type==="wild_packyourbags"){
      ...
      if (order.length>1){
        ...
        for (let i=0;i<order.length;i++){
          const a=nameAt(order[i]), b=nameAt(newOrder[i]);
  ...
  socket.on("disconnect", () => {
    ...
    // If game is running but fewer than 2 seated players remain, stop & reset safely
